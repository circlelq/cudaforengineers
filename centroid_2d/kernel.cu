...
__global__ void centroidKernel(const uchar4 *d_img, int *d_centroidCol,
                               int *d_centroidRow, int *d_pixelCount, int width,
                               int height) {
  ...
  if ((d_img[idx].x < 255 || d_img[idx].y < 255 || d_img[idx].z < 255) &&
      (idx < width * height)) {
    s_img[s_idx].x = col;
    s_img[s_idx].y = row;
    s_img[s_idx].z = 1;
...
void centroidParallel(uchar4 *img, int width, int height) {
  ...
  centroidKernel<<<(width * height + TPB - 1) / TPB, TPB>>>(
      d_img, d_centroidCol, d_centroidRow, d_pixelCount, width, height);
