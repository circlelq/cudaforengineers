...
__device__ bool intersectBox(Ray r, float3 boxmin, float3 boxmax, float *tnear,
                             float *tfar) {
...
__device__ float density(float *d_vol, int3 volSize, float3 pos) {
  ...
  index = make_int3(clipWithBounds(i, 0, w - 2), clipWithBounds(j, 0, h - 2),
...
__device__ uchar4 volumeRenderShader(float *d_vol, int3 volSize, Ray boxRay,
                                     float threshold, int numSteps) {
  uchar4 shade = make_uchar4(96, 0, 192, 0); // background value
...
__device__ uchar4 rayCastShader(float *d_vol, int3 volSize, Ray boxRay,
                                float dist) {
