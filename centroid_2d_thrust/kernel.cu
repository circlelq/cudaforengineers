...
struct PixelFunctor {
  ...
  template <typename T> __host__ __device__ int3 operator()(const T &el) {
...
void centroidParallel(uchar4 *img, int width, int height) {
  ...
  int3 res = thrust::transform_reduce(
      thrust::make_zip_iterator(thrust::make_tuple(first, invec.begin())),
