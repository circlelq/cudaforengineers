...
__device__ float f(float x, float y, float param, float sys) {
  if (sys == 1)
    return x - 2 * param * y; // negative stiffness
  if (sys == 2)
    return -x + param * (1 - x * x) * y; // van der Pol
  else
    return -x - 2 * param * y;
}

// explicit Euler solver
__device__ float2 euler(float x, float y, float dt, float tFinal, float param,
                        float sys) {
  float dx = 0.f, dy = 0.f;
...
__device__ unsigned char clip(float x) {
