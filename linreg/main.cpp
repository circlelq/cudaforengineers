...
int main() {
  ...
  float A[m * n] = {
      ...
      1.0,     8.34,   23.64,   29.74,   19.07,  11.8,    13.97,   22.1,
      14.47,   31.25,  40.77,   58.49,   56.9,   49.69,   40.66,   39.16,
      71.29,   41.76,  69.51,   1010.84, 1011.4, 1007.15, 1007.22, 1017.13,
      1016.05, 1008.2, 1021.98, 1010.25, 90.01,  74.2,    41.91,   76.79,
      97.2,    84.6,   75.38,   78.41,   36.83};
  float b[m] = {480.48, 445.75, 438.76, 453.09, 464.43,
                470.96, 442.35, 464,    428.77};
  ...
  // A = QR with CUSOLVER
  cusolverDnSgeqrf(cusolverDnH, m, n, d_A, lda, tau, work, Lwork, devInfo);
  cudaDeviceSynchronize();

  // z = (Q^T)b with CUSOLVER, z is m x 1
  cusolverDnSormqr(cusolverDnH, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, m, 1, MIN(m, n),
                   d_A, lda, tau, d_b, ldb, work, Lwork, devInfo);
  cudaDeviceSynchronize();

  // Solve Rx = z for x with CUBLAS, x is n x 1.
  cublasStrsm(cublasH, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
              CUBLAS_DIAG_NON_UNIT, n, 1, &alpha, d_A, lda, d_b, ldb);
